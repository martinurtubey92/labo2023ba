...
PARAM <- list()
PARAM$experimento <- "HT13"

PARAM$exp_input <- "TS13"

# En caso que se haga cross validation, se usa esta cantidad de folds
PARAM$lgb_crossvalidation_folds <- 5

PARAM$lgb_semilla <- 100049 # cambiar por su propia semilla
...
PARAM$lgb_basicos <- list(
  ...
  neg_bagging_fraction = 0.45, # 0.0 < neg_bagging_fraction <= 1.0 #SE MODIFICA CONFORME A LO ESTABLECIDO POR EL GRUPO 13B
  is_unbalance = FALSE, #
...
PARAM$bo_lgb <- makeParamSet(
  ...
  makeNumericParam("feature_fraction", lower = 0.02, upper = 0.95),
  makeNumericParam("bagging_fraction" , lower = 0.02, upper = 0.95),
  makeIntegerParam("num_leaves", lower = 2L, upper = 12L),
  makeIntegerParam("min_data_in_leaf", lower = 100L, upper = 50000L),
  ...
  makeIntegerParam("early_stopping_rounds", lower = 10L, upper = 75L)
